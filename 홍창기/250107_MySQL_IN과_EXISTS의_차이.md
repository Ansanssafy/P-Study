# [MySQL] IN과 EXISTS의 차이

> 2024.12.31. 화 작성 (2025.02.12. 수 업데이트)
> 
...
- 따라서 위의 `IN` 조건은 다음과 같이 평가된다.
    - `TRUE OR NULL` 인 경우 → `TRUE`
    - `FALSE OR NULL` 인 경우 → `NULL`
- 즉, 특정 상황에서는 `NULL` 이라는 결과로 인해 이후 과정에서 의도와 다른 결과가 나올 수 있다.
